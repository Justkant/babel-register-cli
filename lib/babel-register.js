...
const yargv = yargs
	...
	.option('inspect', {
		describe: 'Inspect host and/or port.'
	})
	.option('inspect-brk', {
		describe: 'Same as inspect and break at start.'
	})
	.help('h')
	.alias('h', 'help')
	.version(pkg.version)
...

{
	...
	const inspect = yargv.inspect || yargv['inspect-brk'];

	if (inspect) {
		const cmd = yargv.inspect ? 'inspect' : 'inspect-brk';
		args.unshift(`--${cmd}=${inspect}`);
	}
}
